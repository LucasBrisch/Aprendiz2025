...
uses
  ...
  uRecepcionista, System.Generics.Collections, uOperacoes;
...
// Operações [4]
  ...
  end else if cbCargos.ItemIndex=3 then begin
    trabalhador:=TJaguara.Create(nome);
  end else begin
    trabalhador :=Toperacoes.Create(nome);
  end;
...
procedure TFormTrabalhadores.LimparClick(Sender: TObject);
var i : integer;
begin

wlistatrabalhadores := TObjectList<TTrabalhador>.Create;;

for i := ScrollBoxTrabalhadores.ControlCount - 1 downto 0 do
    ScrollBoxTrabalhadores.Controls[i].Free;
