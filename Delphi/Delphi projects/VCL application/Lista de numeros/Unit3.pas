...
procedure TForm3.numeroKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TForm3.RemoverClick(Sender: TObject);
begin
  if Lista.ItemIndex <> -1 then begin
    Lista.Items.Delete(Lista.ItemIndex);
  end else begin
    ShowMessage('Escolha um numero para remover!')
  end;
...
for i := 0 to Lista.Items.Count - 1 do begin
  if TryStrToInt(Lista.Items[i], num) then begin
    if ((num > 30) and (num < 40)) or not ((num > 10) and (num < 100)) then begin
      msg := msg + 'o numero ' + IntToStr (num) + ' é valido!' + sLineBreak;


  end else begin
    msg := msg + 'o numero ' + IntToStr (num) + ' é invalido!' + sLineBreak;
  end;
  end;
